...
const registerUser = asyncHandler(async (req: any, res: any) => {
  const { name, email, course, phone, designation, gender, image } =
    req.body;
...
const getAllUser = asyncHandler(async (req: any, res: any) => {
...
const updateUser = asyncHandler(async (req: any, res: any) => {
  ...
  const { name, email, phone, designation, gender, image, course, isAdmin } =
    req.body;
  try {
...
const deleteEmployee = asyncHandler(async (req: any, res: any) => {
  const { id } = req.params;
  const user = await Employee.findById(id);
  if (!user) {
    res.status(404).json({ message: "User not found" });
